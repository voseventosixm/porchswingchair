...
static void bs_rdwr_request(struct scsi_cmd *cmd)
{
	...
	int i;
	char *ptr;
	struct sg_io_hdr scsiPassThrough;
...
verify:
	...
	case ATA_PASS_THROUGH_16:
        devinfo.desc = fd;
        devinfo.name = cmd->dev->path;

		switch (cmd->scb[14]) {
			case ATA_IDENTIFY_DEVICE: 
                handle_identify_command(devinfo, scsi_get_in_buffer(cmd)); 
                break;

			case ATA_SMART:
                if (0xD0 == cmd->scb[4]) {
                    handle_smart_command(devinfo, scsi_get_in_buffer(cmd));
                } else if (0xD1 == cmd->scb[4]) {
            ...
            case ATA_SET_FEATURE:
                if(0x56 == cmd->scb[4]) {
                    sampling_rate = (cmd->scb[7] << 8) + cmd->scb[8];
                    handle_set_sampling_rate_command(devinfo, sampling_rate);
                }
            break;
