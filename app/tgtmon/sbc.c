...
static int sbc_ata_pass_through(int host_no, struct scsi_cmd *cmd)
{
	int ret;
	uint16_t asc = ASC_LUN_NOT_SUPPORTED;
	uint32_t sectorCount;
	uint64_t lba;
	uint8_t ataCommand = 0x00;
	unsigned char key = ILLEGAL_REQUEST;

	switch (cmd->scb[0]) {
		case ATA_PASS_THROUGH_16:
			ataCommand = cmd->scb[14];
			// TODO: Handle extended commands
			lba = ((uint64_t)cmd->scb[12] << 16 | (uint64_t)cmd->scb[10] << 8 | (uint64_t)cmd->scb[8]);
			sectorCount = ((uint16_t)cmd->scb[6]);
			break;
		default:
			goto sense;
			break;
	...
	switch (ataCommand) {
		case ATA_IDENTIFY_DEVICE:
			sectorCount = 1;
			cmd->offset = 0;
			cmd->tl = (sectorCount << cmd->dev->blk_shift);
			scsi_set_in_resid_by_actual(cmd, cmd->tl);
			break;
		case ATA_SMART:
			// TODO: Properly handle SMART subcommand
            sectorCount = 1;
            cmd->offset = 0;

            // Virtium read prime log
            if (0xD5 == cmd->scb[4]) sectorCount = cmd->scb[6];
...
static struct device_type_template sbc_template = {
	...
	.ops		= {
		...
		{spc_illegal_op,},
		{spc_illegal_op,},
		{sbc_ata_pass_through}, // ATA_PASS_THROUGH_16
		{spc_illegal_op,},
		{spc_illegal_op,},
		{spc_illegal_op,},
